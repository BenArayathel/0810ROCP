...
public class Driver {
	...
	public static final String ORDER_TOTAL = "Total for order:";
	...
	public static void main(String[] args) {
		...
		Menu menu = makeMenu();
		//currently have this in present menu, but need to access menu in other location. 
	...
	private static void displayOrder(Order order) {
		...
		for (Map.Entry<FoodItem, Integer> entry: order.getOrderContents().entrySet()) {
			System.out.println(entry.getValue() + " " +  entry.getKey().getName() + "	$" + entry.getKey().getCost() + " (each)");
		}
	...
	public static Menu makeMenu() { 
		...
		FoodItem veggieBurger = new FoodItem("Veggie Burger", "Entree","Our House-made Veggie Burgers, served with hand-cut fries.",10);
		...
		FoodItem cococadoShake = new FoodItem("Cococado Vegan Milkshake","Dessert", "Made with rice dream ice cream, bananas, coconut and avocado. You know you love it!",4);
